...
-- Export MySQL : tout ce qui n'est pas une requête solution est en
-- commentaire (--), afin que ce fichier puisse être exécuté tel quel
...
SELECT nom, prenom, groupe_sanguin FROM patients;
...
SELECT id_patient, nom, age FROM patients;
...
SELECT * FROM patients WHERE service = 'Cardiologie';
...
SELECT * FROM patients WHERE sexe = 'M';
...
SELECT * FROM patients WHERE service IN ('Cardiologie', 'Neurologie') AND age BETWEEN 30 AND 60;
...
SELECT * FROM patients WHERE sexe = 'F' AND service IN ('Gériatrie', 'Oncologie');
...
SELECT * FROM medecins WHERE salaire BETWEEN 8000 AND 9000;
...
SELECT id_patient, chambre, date_entree FROM hospitalisations WHERE date_sortie IS NULL;
...
SELECT id_patient, chambre, date_entree, date_sortie FROM hospitalisations WHERE date_sortie IS NOT NULL;
...
SELECT id_hosp, chambre, date_entree, COALESCE(date_sortie, 'En cours') AS statut FROM hospitalisations;
...
SELECT nom, prenom, age, service FROM patients ORDER BY age DESC LIMIT 3;

-- Exercice 2 : Les 5 consultations les plus longues : diagnostic, duree_min, medecin_id.
--   Indice 1 : ORDER BY duree_min DESC
--   Indice 2 : LIMIT 5
SELECT diagnostic, duree_min, medecin_id FROM consultations ORDER BY duree_min DESC LIMIT 5;

-- Exercice 3 : Les 3 médecins les mieux payés : nom, specialite, salaire.
--   Indice 1 : ORDER BY salaire DESC
--   Indice 2 : LIMIT 3
SELECT nom, specialite, salaire FROM medecins ORDER BY salaire DESC LIMIT 3;
...
SELECT DISTINCT service, statut FROM patients;
...
SELECT nom, prenom, age FROM patients ORDER BY age DESC LIMIT 4 OFFSET 1;
...
SELECT id_consultation, diagnostic, duree_min FROM consultations ORDER BY id_consultation LIMIT 3 OFFSET 3;
...
SELECT nom_medicament, prix_unitaire FROM medicaments ORDER BY prix_unitaire ASC LIMIT 1 OFFSET 2;
...
SELECT nom, prenom, age FROM patients ORDER BY age DESC LIMIT 2;
...
SELECT diagnostic, duree_min FROM consultations ORDER BY duree_min DESC LIMIT 5;
...
SELECT nom, prenom, age FROM patients ORDER BY age DESC LIMIT (SELECT CAST(COUNT(*) * 0.30 AS INT) FROM patients);
...
SELECT UPPER(nom) AS nom_maj, LOWER(prenom) AS prenom_min, LENGTH(nom) AS longueur_nom FROM patients;
...
SELECT CONCAT(prenom, ' ', nom) AS nom_complet, MONTH(date_admission) AS mois FROM patients;
...
SELECT nom_medicament, prix_unitaire, ROUND(prix_unitaire, 1) AS prix_arrondi FROM medicaments;
...
SELECT p.nom, c.date_consultation, c.diagnostic FROM patients p INNER JOIN consultations c ON p.id_patient = c.id_patient;
...
SELECT p.nom, h.chambre, h.date_entree FROM patients p INNER JOIN hospitalisations h ON p.id_patient = h.id_patient;
...
SELECT m.nom AS medecin, c.diagnostic, c.duree_min FROM consultations c INNER JOIN medecins m ON c.medecin_id = m.id_medecin;
...
--   Indice 2 : ON p.id_patient = c.id_patient
SELECT p.nom, p.prenom, c.diagnostic FROM patients p LEFT JOIN consultations c ON p.id_patient = c.id_patient;
...
SELECT p.nom, p.prenom, h.chambre FROM patients p LEFT JOIN hospitalisations h ON p.id_patient = h.id_patient;
...
SELECT m.nom AS medecin, c.diagnostic FROM medecins m LEFT JOIN consultations c ON m.id_medecin = c.medecin_id;
...
SELECT COUNT(*) AS nb, AVG(age) AS age_moyen, MAX(age) AS age_max, MIN(age) AS age_min FROM patients;
...
SELECT SUM(duree_min) AS total_min, AVG(duree_min) AS duree_moy, MAX(duree_min) AS duree_max FROM consultations;
...
SELECT AVG(cout_journalier) AS moy, MIN(cout_journalier) AS min_cout, MAX(cout_journalier) AS max_cout FROM hospitalisations;
...
SELECT service, COUNT(*) AS nb_patients, AVG(age) AS age_moyen FROM patients GROUP BY service ORDER BY nb_patients DESC;
...
SELECT medecin_id, COUNT(*) AS nb_consultations, AVG(duree_min) AS duree_moy FROM consultations GROUP BY medecin_id ORDER BY nb_consultations DESC;
...
SELECT service, GROUP_CONCAT(nom, ', ') AS liste_patients FROM patients GROUP BY service;
...
SELECT service, COUNT(*) AS nb FROM patients GROUP BY service HAVING COUNT(*) >= 2;
...
--   Indice 2 : HAVING COUNT(*) >= 2
SELECT medecin_id, COUNT(*) AS nb_consultations FROM consultations GROUP BY medecin_id HAVING COUNT(*) >= 2;
...
SELECT id_medicament, SUM(quantite) AS qte_totale FROM prescriptions GROUP BY id_medicament HAVING SUM(quantite) > 30;
...
SELECT p.nom, c.diagnostic, m.nom AS medecin FROM patients p JOIN consultations c ON p.id_patient = c.id_patient JOIN medecins m ON c.medecin_id = m.id_medecin;
...
SELECT p.nom, c.duree_min, m.nom AS medecin, m.specialite FROM patients p JOIN consultations c ON p.id_patient = c.id_patient JOIN medecins m ON c.medecin_id = m.id_medecin;
...
SELECT p.nom, p.service, m.nom AS medecin, m.specialite FROM patients p JOIN medecins m ON p.medecin_id = m.id_medecin;
...
SELECT * FROM consultations WHERE diagnostic LIKE '%cardiaque%';
...
SELECT * FROM medicaments WHERE categorie LIKE 'Anti%';
...
SELECT diagnostic, duree_min, medecin_id FROM consultations WHERE duree_min > (SELECT AVG(duree_min) FROM consultations);
...
SELECT nom, specialite, salaire FROM medecins WHERE salaire > (SELECT AVG(salaire) FROM medecins);
...
WITH patients_seniors AS (SELECT * FROM patients WHERE age >= 65) SELECT COUNT(*) AS nb_consultations FROM patients_seniors p JOIN consultations c ON p.id_patient = c.id_patient;
...
WITH consult_longues AS (SELECT * FROM consultations WHERE duree_min > 40) SELECT c.diagnostic, m.nom AS medecin FROM consult_longues c JOIN medecins m ON c.medecin_id = m.id_medecin;
...
WITH medecins_top AS (SELECT * FROM medecins WHERE salaire > 8500) SELECT p.nom, p.service, mt.nom AS medecin FROM patients p JOIN medecins_top mt ON p.medecin_id = mt.id_medecin;
...
SELECT nom, service, CASE WHEN service IN ('Oncologie', 'Cardiologie') THEN 'Haute' ELSE 'Normale' END AS priorite FROM patients;
...
SELECT nom, age, CASE WHEN age < 40 THEN 'Jeune' WHEN age <= 65 THEN 'Adulte' ELSE 'Senior' END AS tranche_age FROM patients;
...
SELECT diagnostic, duree_min, CASE WHEN duree_min < 30 THEN 'Courte' WHEN duree_min <= 50 THEN 'Normale' ELSE 'Longue' END AS duree_cat FROM consultations;
...
SELECT id_patient, COUNT(*) AS nb_consultations FROM consultations GROUP BY id_patient HAVING COUNT(*) > 1;
...
--   Indice 2 : GROUP BY id_patient
SELECT id_patient, COUNT(*) AS nb_hospitalisations FROM hospitalisations GROUP BY id_patient HAVING COUNT(*) > 1;
...
SELECT id_medicament, COUNT(*) AS nb_fois FROM prescriptions GROUP BY id_medicament HAVING COUNT(*) > 1;
...
SELECT p.nom, p.prenom FROM patients p LEFT JOIN consultations c ON p.id_patient = c.id_patient WHERE c.id_consultation IS NULL;

-- Exercice 2 : Listez les patients qui n'ont aucune hospitalisation enregistrée.
--   Indice 1 : LEFT JOIN hospitalisations h ON p.id_patient = h.id_patient
--   Indice 2 : WHERE h.id_hosp IS NULL
SELECT p.nom, p.prenom FROM patients p LEFT JOIN hospitalisations h ON p.id_patient = h.id_patient WHERE h.id_hosp IS NULL;
...
SELECT nom, specialite FROM medecins WHERE id_medecin NOT IN (SELECT medecin_id FROM consultations);
...
SELECT MONTH(date_consultation) AS mois, COUNT(*) AS nb FROM consultations GROUP BY mois ORDER BY mois;
...
--   Indice 2 : GROUP BY mois
SELECT MONTH(date_admission) AS mois, COUNT(*) AS nb_admissions FROM patients GROUP BY mois ORDER BY mois;
...
SELECT date_consultation, diagnostic, duree_min FROM consultations WHERE MONTH(date_consultation) = 11;
...
SELECT id_patient, chambre, date_entree, COALESCE(date_sortie, 'Toujours hospitalisé') AS date_sortie FROM hospitalisations;
...
SELECT id_hosp, chambre, cout_journalier, COALESCE(date_sortie, 'En cours') AS statut_sortie FROM hospitalisations ORDER BY id_hosp;
...
SELECT nom_medicament, prix_unitaire, NULLIF(prix_unitaire, 0.15) AS prix_ajuste FROM medicaments;
...
SELECT medecin_id, diagnostic, duree_min, RANK() OVER (PARTITION BY medecin_id ORDER BY duree_min DESC) AS rang FROM consultations ORDER BY medecin_id, rang;
...
SELECT nom, age, ROW_NUMBER() OVER (ORDER BY age DESC) AS rang FROM patients;
...
SELECT nom, service, age, RANK() OVER (PARTITION BY service ORDER BY age DESC) AS rang FROM patients ORDER BY service, rang;
...
SELECT date_consultation, duree_min, SUM(duree_min) OVER (ORDER BY date_consultation) AS cumul_minutes FROM consultations ORDER BY date_consultation;
...
SELECT id_hosp, chambre, cout_journalier, SUM(cout_journalier) OVER (ORDER BY id_hosp) AS cumul_cout FROM hospitalisations;
...
SELECT date_consultation, duree_min, AVG(duree_min) OVER (ORDER BY date_consultation) AS moy_glissante FROM consultations ORDER BY date_consultation;
...
SELECT date_consultation, duree_min, LAG(duree_min, 1) OVER (ORDER BY date_consultation) AS duree_precedente FROM consultations ORDER BY date_consultation;
...
SELECT date_consultation, duree_min, LEAD(duree_min, 1) OVER (ORDER BY date_consultation) AS duree_suivante FROM consultations ORDER BY date_consultation;
...
SELECT medecin_id, date_consultation, duree_min, LAG(duree_min, 1) OVER (PARTITION BY medecin_id ORDER BY date_consultation) AS duree_prec FROM consultations ORDER BY medecin_id, date_consultation;
...
SELECT MAX(salaire) AS deuxieme_salaire FROM medecins WHERE salaire < (SELECT MAX(salaire) FROM medecins);
...
SELECT DISTINCT duree_min AS deuxieme_duree FROM consultations ORDER BY duree_min DESC LIMIT 1 OFFSET 1;
...
SELECT DISTINCT age AS troisieme_age FROM patients ORDER BY age DESC LIMIT 1 OFFSET 2;
...
CREATE INDEX idx_service ON patients(service);
...
EXPLAIN SELECT * FROM consultations WHERE medecin_id = 2;
...
CREATE INDEX idx_patient_consult ON consultations(id_patient);
...
SELECT nom FROM patients UNION SELECT nom FROM medecins;
...
SELECT nom FROM patients UNION ALL SELECT nom FROM medecins;
...
SELECT nom, 'Patient' AS type FROM patients UNION SELECT nom, 'Médecin' AS type FROM medecins ORDER BY type;
...
SELECT p.nom, p.prenom FROM patients p WHERE EXISTS (SELECT 1 FROM consultations c WHERE c.id_patient = p.id_patient);
...
SELECT p.nom, p.prenom FROM patients p WHERE NOT EXISTS (SELECT 1 FROM hospitalisations h WHERE h.id_patient = p.id_patient);
...
SELECT m.nom, m.specialite FROM medecins m WHERE EXISTS (SELECT 1 FROM consultations c WHERE c.medecin_id = m.id_medecin);
...
SELECT nom, UPPER(specialite) AS specialite_maj, LENGTH(nom) AS longueur_nom FROM medecins;
...
SELECT nom, SUBSTRING(nom, 1, 3) AS debut_nom FROM patients;
...
SELECT CONCAT(prenom, ' ', nom) AS nom_complet FROM patients;
...
SELECT CONCAT(nom, ' - ', specialite) AS fiche FROM medecins;
...
SELECT CONCAT(nom, ' (', service, ')') AS patient_service FROM patients;
...
INSERT INTO patients (nom, prenom, age, sexe, service) VALUES ('Dupont', 'Alice', 45, 'F', 'Cardiologie');
...
INSERT INTO medicaments (id_medicament, nom_medicament, categorie, prix_unitaire) VALUES (8, 'Paracétamol', 'Antalgique', 0.05); SELECT * FROM medicaments;
...
INSERT INTO prescriptions (id_prescription, id_consultation, id_medicament, quantite, duree_jours) VALUES (8, 1, 7, 20, 30); SELECT * FROM prescriptions;
...
UPDATE patients SET statut = 'Hospitalisé' WHERE id_patient = 3; SELECT nom, statut FROM patients WHERE id_patient = 3;
...
UPDATE medecins SET salaire = salaire + 500 WHERE id_medecin = 1; SELECT nom, salaire FROM medecins WHERE id_medecin = 1;
...
UPDATE hospitalisations SET date_sortie = '2024-11-20' WHERE id_hosp = 2; SELECT * FROM hospitalisations WHERE id_hosp = 2;
...
DELETE FROM prescriptions WHERE duree_jours < 20; SELECT * FROM prescriptions;
...
DELETE FROM hospitalisations WHERE date_sortie < '2024-11-01'; SELECT * FROM hospitalisations;
...
DELETE FROM consultations WHERE medecin_id = 5; SELECT * FROM consultations;
...
CREATE VIEW vue_seniors AS SELECT nom, prenom, age, service FROM patients WHERE age >= 65; SELECT * FROM vue_seniors;
...
CREATE VIEW vue_consultations_longues AS SELECT * FROM consultations WHERE duree_min > 40; SELECT diagnostic, duree_min FROM vue_consultations_longues;
...
CREATE VIEW vue_patients_medecins AS SELECT p.nom AS patient, p.service, m.nom AS medecin FROM patients p JOIN medecins m ON p.medecin_id = m.id_medecin; SELECT * FROM vue_patients_medecins;
...
CREATE TABLE notes_medicales (id_note INTEGER, id_patient INTEGER, note TEXT, date_note TEXT);
...
ALTER TABLE patients ADD telephone TEXT; SELECT nom, telephone FROM patients LIMIT 3;
...
CREATE TABLE bilans_sanguins (id_bilan INTEGER, id_patient INTEGER, date_bilan TEXT, resultat TEXT); SELECT * FROM bilans_sanguins;
...
SELECT nom, salaire, DENSE_RANK() OVER (ORDER BY salaire DESC) AS rang, NTILE(3) OVER (ORDER BY salaire) AS groupe FROM medecins;
...
SELECT nom, age, NTILE(4) OVER (ORDER BY age) AS quartile FROM patients;
...
SELECT nom, age, DENSE_RANK() OVER (ORDER BY age DESC) AS rang FROM patients ORDER BY rang;
...
SELECT p.nom, p.service, m.salaire, FIRST_VALUE(m.salaire) OVER (PARTITION BY p.service ORDER BY m.salaire) AS salaire_min FROM patients p JOIN medecins m ON p.service = m.service;
...
SELECT medecin_id, diagnostic, duree_min, FIRST_VALUE(duree_min) OVER (PARTITION BY medecin_id ORDER BY duree_min) AS duree_min_medecin FROM consultations ORDER BY medecin_id;
...
SELECT nom, service, age, FIRST_VALUE(age) OVER (PARTITION BY service ORDER BY age) AS plus_jeune_service FROM patients ORDER BY service;
...
SELECT AVG(salaire) AS moyenne, VARIANCE(salaire) AS variance, STDDEV(salaire) AS ecart_type FROM medecins;
...
SELECT AVG(duree_min) AS moy, VARIANCE(duree_min) AS variance, STDDEV(duree_min) AS ecart_type FROM consultations;
...
SELECT AVG(age) AS age_moyen, VARIANCE(age) AS variance, STDDEV(age) AS ecart_type FROM patients;
...
SELECT nom, salaire, PERCENT_RANK() OVER (ORDER BY salaire) AS percentile FROM medecins;
...
SELECT nom, age, PERCENT_RANK() OVER (ORDER BY age) AS percentile_age FROM patients;
...
SELECT diagnostic, duree_min, PERCENT_RANK() OVER (ORDER BY duree_min) AS percentile_duree FROM consultations;
...
SELECT m.nom AS medecin, COUNT(c.id_consultation) AS nb_consult, AVG(c.duree_min) AS duree_moy, SUM(c.duree_min) AS duree_totale FROM medecins m JOIN consultations c ON m.id_medecin = c.medecin_id GROUP BY m.nom HAVING COUNT(*) >= 2 ORDER BY duree_totale DESC;
...
SELECT p.service, COUNT(DISTINCT p.id_patient) AS nb_patients, SUM(c.duree_min) AS total_min FROM patients p JOIN consultations c ON p.id_patient = c.id_patient GROUP BY p.service ORDER BY nb_patients DESC;
...
SELECT m.nom AS medecin, COUNT(pr.id_prescription) AS nb_prescriptions FROM medecins m JOIN consultations c ON m.id_medecin = c.medecin_id JOIN prescriptions pr ON c.id_consultation = pr.id_consultation GROUP BY m.nom HAVING COUNT(pr.id_prescription) >= 1 ORDER BY nb_prescriptions DESC;
...
SELECT service, COUNT(CASE WHEN sexe='M' THEN 1 END) AS hommes, COUNT(CASE WHEN sexe='F' THEN 1 END) AS femmes FROM patients GROUP BY service ORDER BY service;
...
SELECT medecin_id, COUNT(CASE WHEN duree_min < 30 THEN 1 END) AS courtes, COUNT(CASE WHEN duree_min >= 30 THEN 1 END) AS longues FROM consultations GROUP BY medecin_id;
...
SELECT COUNT(CASE WHEN statut='Hospitalisé' THEN 1 END) AS hospitalises, COUNT(CASE WHEN statut='Ambulatoire' THEN 1 END) AS ambulatoires FROM patients;
...
--   Indice 2 : INTERSECT
SELECT id_patient FROM consultations INTERSECT SELECT id_patient FROM hospitalisations;
...
SELECT id_patient FROM patients EXCEPT SELECT id_patient FROM consultations;
...
SELECT nom FROM patients EXCEPT SELECT nom FROM medecins ORDER BY nom;
...
SELECT NOW() AS maintenant, CURDATE() AS aujourd_hui;
...
SELECT id_hosp, chambre, date_entree, date_sortie, DATEDIFF(date_sortie, date_entree) AS duree_jours FROM hospitalisations WHERE date_sortie IS NOT NULL;
...
SELECT diagnostic, DATE_FORMAT(date_consultation, '%d/%m/%Y') AS date_fr FROM consultations;
...
SELECT TRIM(nom) AS nom_propre, LEFT(nom, 1) AS initiale FROM patients;
...
SELECT diagnostic, REPLACE(diagnostic, 'artérielle', 'art.') AS diagnostic_court FROM consultations;
...
SELECT nom, groupe_sanguin, LEFT(groupe_sanguin, 2) AS type_sang, RIGHT(groupe_sanguin, 1) AS rhesus FROM patients;
